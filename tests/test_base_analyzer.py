...
class TestBaseAnalyser:
    ...
    def test_apply_filters(self, ana1):
        ...
        r = base.IntramolecularAnalyzer._apply_filters(t)
        pd.testing.assert_frame_equal(r, t[~(f1 | f1_neg | f2)])
        r = base.IntramolecularAnalyzer._apply_filters(t, type="mask")
        np.testing.assert_array_equal(r, ~(f1 | f1_neg | f2))
        r = base.IntramolecularAnalyzer._apply_filters(
        ...
        r = base.IntramolecularAnalyzer._apply_filters(t, ignore="f1")
        pd.testing.assert_frame_equal(r, t[~f2])
        r = base.IntramolecularAnalyzer._apply_filters(
            t, ignore="f1", type="mask")
        np.testing.assert_array_equal(r, ~f2)
        r = base.IntramolecularAnalyzer._apply_filters(t, ignore="f2")
        ...
        r = base.IntramolecularAnalyzer._apply_filters(t, skip_neighbors=False)
        pd.testing.assert_frame_equal(r, t[~(f1 | f1_neg | f2)])
        r = base.IntramolecularAnalyzer._apply_filters(t, skip_neighbors=True)
    ...
    def test_save_load(self, tracker, tmp_path):
        ...
        with io.chdir(tmp_path):
            ana.save("my_prefix")
            ana2 = base.IntramolecularAnalyzer.load(
                "my_prefix", reset_filters=False)
            ana2_nof = base.IntramolecularAnalyzer.load(
                "my_prefix", reset_filters=True)
            tracker2 = base.IntramolecularTracker.load("my_prefix")
    ...
    def test_from_tracker(self, tracker):
        ana = base.IntramolecularAnalyzer.from_tracker(tracker)
        self._check_analyzer_from_tracker(ana, tracker)
